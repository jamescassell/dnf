    ...
    def get(self, *patterns):
        ...
        for pat in patterns:
            ...
            if not envs and not grps:
                if self.status == self.INSTALLED:
                    msg = _("Warning: Module or Group '%s' is not installed.") % ucd(pat)
                elif self.status == self.AVAILABLE:
                    msg = _("Warning: Module or Group '%s' is not available.") % ucd(pat)
                else:
                    msg = _("Warning: Module or Group '%s' does not exist.") % ucd(pat)
...
class Solver(object):
    ...
    def _group_upgrade(self, group_id):
        ...
        if not swdb_group:
            ...
            raise CompsError(_("Warning: Module or Group '%s' is not installed.") % argument)
        if not comps_group:
            raise CompsError(_("Warning: Module or Group '%s' is not available.") % group_id)
